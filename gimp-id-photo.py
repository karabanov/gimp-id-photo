...
class id_photo_base(object):
  ...
  def print_functon(self, image, drawable, paper, copys, print_photo):
    ...
    # Ensure drawable is not None and has width/height attributes
    if drawable is None or not hasattr(drawable, "width") or not hasattr(drawable, "height"):
      self.info("Ошибка: Не удалось получить размеры слоя для печати. Проверьте, что выбран корректный слой.")
      return
    layer_new_width = drawable.width + space
    layer_new_height = drawable.height + space
    # Вычисляем сколько фоток по вертикали и горизонтала поместится
    # на заданном холсте
    ...
    # Присвоим слою источник адекватное имя
    drawable.name = 'Фото'
    ...
    for i in range(h_count):
      for j in range(w_count):
        ...
        # то выходим из цикла при достижении максимного (w_count * h_count) числа копий,
...
class select_format_id_photo(id_photo_base):
  def __init__(self, runmode, image, drawable):
    ...
    self.pass_vbox = Gtk.VBox(homogeneous=False, spacing=5)
    self.pass_vbox.set_border_width(5)
    self.pass_vbox.show()
    # Вертикальный бокс для виз
    self.visa_vbox = Gtk.VBox(homogeneous=False, spacing=5)
    self.visa_vbox.set_border_width(5)
    self.visa_vbox.show()
    # Вертикальный бокс для удостоверений
    self.cert_vbox = Gtk.VBox(homogeneous=False, spacing=5)
    self.cert_vbox.set_border_width(5)
    self.cert_vbox.show()
    # Вертикальный бокс для иных форматов
    self.other_vbox = Gtk.VBox(homogeneous=False, spacing=5)
    ...
    # self.learn_more_alignment = Gtk.Alignment.new(0.0, 0.0, 0.0, 0.0)
    # self.learn_more_alignment.add(self.learn_more_label)
    # self.learn_more_alignment.show()
    self.learn_more_alignment = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL)
    self.learn_more_alignment.set_halign(Gtk.Align.START)
    self.learn_more_alignment.pack_start(self.learn_more_label, False, False, 0)
    self.learn_more_alignment.show()
    ...
    self.table.attach(self.learn_more_alignment, 0, 2, 4, 1)
    self.table.show()
    ...
    self.window.show()
    Gtk.main()  # <-- Add this line to start the GTK main loop
  # Эта функция крафтит фото для документов
  def auto_execute(self, widget, data=None):
    # Скрываем окно, чтоб не мешало
    self.window.hide()
